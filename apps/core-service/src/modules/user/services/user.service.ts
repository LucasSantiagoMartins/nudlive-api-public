...
import { HashPasswordUtils } from '@nudlive/common/utils/hash-password.utils';
import { ProfileService } from '../../profile/services/profile.service';
import { KafkaService } from '@nudlive/kafka';
import { EventTopics, UserCreatedEvent, UserUpdatedEvent } from '@nudlive/events';
...
export class UserService {
  ...
  async createFromGoogle(profile: any): Promise<User> {
    ...
    try {
      ...
      const userWithProfile = await this.userQueryService.findByIdWithProfile(savedUser.id);
      ...
      await this.kafkaService.publish(
        ...
        new UserCreatedEvent(
          ...
          savedUser.phoneNumber,
          savedUser.role,
          userWithProfile.profile?.fullName || '',
          profilePhotoUrl
        ),
      );
  ...
  async convertToCreator(userId: number): Promise<{ token: string }> {
    ...
    await this.kafkaService.publish(
      ...
      user.id.toString(),
      'UserUpdatedEvent',
      new UserUpdatedEvent(user.id, user.role),
    );
